...
export const handler: APIGatewayProxyHandler = async (event) => {
    try {
        ...
        for (let i = 0; i < listaDeItens.length; i++) {
            ...
            let valorDoItem = UTILS.moedaParaFloat((listaDeItens[i] as { valorDoItem: string }).valorDoItem);
            let totalDoItem = UTILS.moedaParaFloat((listaDeItens[i] as { totalDoItem: string }).totalDoItem);
            ...
            xmlContent += '<TITMMOV>'
                ...
                xmlContent += XML.montaTag('PRECOTABELA', '0');
                xmlContent += XML.montaTag('DATAEMISSAO', date.getDateTime());
                xmlContent += XML.montaTag('CODUND', '');
                xmlContent += XML.montaTag('QUANTIDADEARECEBER', qtdDoItem);
                xmlContent += XML.montaTag('VALORUNITARIO', '0');
                xmlContent += XML.montaTag('VALORFINANCEIRO', '0');
                xmlContent += XML.montaTag('CODCCUSTO', campos.codigoDoCentroDeCusto);
                xmlContent += XML.montaTag('ALIQORDENACAO', '0');
                xmlContent += XML.montaTag('QUANTIDADEORIGINAL', qtdDoItem);
                xmlContent += XML.montaTag('FLAG', '0');
                xmlContent += XML.montaTag('FATORCONVUND', '0');
                xmlContent += XML.montaTag('VALORBRUTOITEM', totalDoItem); // PRECO UNITARIO X QTD
                xmlContent += XML.montaTag('VALORTOTALITEM', '0');
                xmlContent += XML.montaTag('QUANTIDADESEPARADA', totalDoItem); // PRECO UNITARIO X QTD
                xmlContent += XML.montaTag('COMISSAOREPRES', '0');
                xmlContent += XML.montaTag('VALORESCRITURACAO', '0');
                xmlContent += XML.montaTag('VALORFINPEDIDO', '0');
                xmlContent += XML.montaTag('VALOROPFRM1', '0');
                xmlContent += XML.montaTag('VALOROPFRM2', '0');
                ...
                xmlContent += XML.montaTag('VALORDESCCONDICONALITM', '0');
                xmlContent += XML.montaTag('VALORDESPCONDICIONALITM', '0');
                xmlContent += XML.montaTag('CODTBORCAMENTO', campos.codigoDaNatureza);
                xmlContent += XML.montaTag('CODCOLTBORCAMENTO', '0');
                xmlContent += XML.montaTag('RATEIOFRETE', '0');
                xmlContent += XML.montaTag('RATEIODESC', '0');
                xmlContent += XML.montaTag('RATEIODESP', '0');
                xmlContent += XML.montaTag('VALORUNTORCAMENTO', '0');
                xmlContent += XML.montaTag('VALSERVICONFE', '0');
                xmlContent += XML.montaTag('CODLOC', ESTOQUE);
                xmlContent += XML.montaTag('VALORBEM', '0');
                ...
                xmlContent += XML.montaTag('QUANTIDADETOTAL', qtdDoItem);
            ...
            for (let j = 0; j < tributos.length; j++) {
                xmlContent += '<TTRBITMMOV>';
                    ...
                    xmlContent += XML.montaTag('VALOR', '0');    
                    xmlContent += XML.montaTag('FATORREDUCAO', '0');
                    xmlContent += XML.montaTag('FATORSUBSTTRIB', '0');
                    xmlContent += XML.montaTag('BASEDECALCULOCALCULADA', totalDoItem);
                    xmlContent += XML.montaTag('EDITADO', '0');
                    xmlContent += XML.montaTag('PERCDIFERIMENTOPARCIALICMS', '0');
                    xmlContent += XML.montaTag('BASECHEIA', '0');
                xmlContent += '</TTRBITMMOV>';
            }
            xmlContent += '<TITMMOVFISCAL>'
                ...
                xmlContent += XML.montaTag('IDMOV', '-1');
                xmlContent += XML.montaTag('NSEQITMMOV', (i + 1).toString());
                xmlContent += XML.montaTag('QTDECONTRATADA', '0');
                xmlContent += XML.montaTag('VLRTOTTRIB', '0');
        ...
        cData += '<![CDATA['
            cData += '<MovMovimento>'
                cData += '<TMOV>'
                    ...
                    cData += XML.montaTag('TIPO', campos.tipoDeSolicitacao)
                    ...
                    cData += XML.montaTag('PERCENTUALFRETE', '0')
                    cData += XML.montaTag('VALORFRETE', '0')
                    cData += XML.montaTag('PERCENTUALDESC', '0')
                    cData += XML.montaTag('VALORDESC', '0')
                    cData += XML.montaTag('PERCENTUALDESP', '0')
                    cData += XML.montaTag('VALORDESP', '0')
                    cData += XML.montaTag('PERCCOMISSAO', '0')
                    cData += XML.montaTag('CODMEN', '01')
                    cData += XML.montaTag('PESOLIQUIDO', '0')
                    cData += XML.montaTag('PESOBRUTO', '0')
                    ...
                    cData += XML.montaTag('STSEMAIL', '0')
                    cData += XML.montaTag('VALORBRUTOINTERNO', '0.0000')
                    cData += XML.montaTag('VINCULADOESTOQUEFL', '0')
                    ...
                    cData += XML.montaTag('INTEGRADOAUTOMACAO', '0')
                    cData += XML.montaTag('INTEGRAAPLICACAO', 'T')
                    cData += XML.montaTag('RECIBONFESTATUS', '0')
                    cData += XML.montaTag('VALORMERCADORIAS', '0.0000')
                    cData += XML.montaTag('USARATEIOVALORFIN', '0')
                    cData += XML.montaTag('CODCOLCFOAUX', '0')
                    cData += XML.montaTag('HISTORICOCURTO', HISTORICOCURTO)
                    cData += XML.montaTag('RATEIOCCUSTODEPTO', '0.0000')
                    cData += XML.montaTag('VALORBRUTOORIG', '0.0000')
                    cData += XML.montaTag('VALORLIQUIDOORIG', '0.0000')
                    cData += XML.montaTag('VALOROUTROSORIG', '0.0000')
        ...
        let result = await dataServer.saveRecord(cData, 'MovMovimento', 'MovMovimento');
